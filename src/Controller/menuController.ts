...
const readMenu = async (req: Request, res: Response) => {
    try {
        ...
        const jenis = req.query.jenis?.toString(); // FILTER MAKANAN | MINUMAN
        const stanId = req.query.stan ? Number(req.query.stan) : null;  // FILTER STAN
        ...
        const result = allMenu.map(menu => {
            ...
            const [diskonAktif] = menu.menu_diskonDetail 
